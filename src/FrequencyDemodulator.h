...
public:
    ...
    void analytic_signal(double waveform_real[BUFFER_SIZE],
                         double waveform_imag[BUFFER_SIZE],
                         bool window = true);

    // Calculates the instantaneous frequencies of a waveform
    // and stores the result in frequency_data[].
    void frequencies(const double waveform_data[BUFFER_SIZE],
                     double frequency_data[BUFFER_SIZE-1]);

private:

    double instantaneous_frequency(double real1, double imag1,
